...
@php
    // 確保 $buyerRatings 至少是一個集合，避免 null 造成錯誤
          ...
          <tbody>
            @foreach($buyerOrders as $order)
              @php
                  ...
                  // 面交資訊：優先訂單，其次商品預設，最後給空陣列
                  $locationData = $order->meetup_location ?: ($item?->meetup_location ?? []);
                  ...
                  $seller = $item?->user;
                  $sellerName = optional($seller)->nickname
              ...
              <tr>
                ...
                <td data-label="價格">
                  NT$ {{ number_format($order->order_price ?? 0, 0) }}
                </td>

                <td data-label="交易地點">{{ $location }}</td>

                <td data-label="交易時間">{{ $datetime }}</td>
                ...
                <td data-label="操作" class="text-end">
                      ...
                      <div class="order-edit-card">
                        <form method="POST" action="{{ route('orders.update', $order) }}" class="stack gap-2">
                          ...
                          <div class="form-group">
                            ...
                            <input
                              id="buyer-address-{{ $order->id }}"
                              type="text"
                              name="meet_address"
                              value="{{ data_get($order->meetup_location, 'address') }}"
                              class="form-control"
                              required
                            >
                          </div>
                          ...
                          <div class="d-flex gap-2">
                            <div class="form-group flex-fill">
                              ...
                              <input
                                id="buyer-date-{{ $order->id }}"
                                type="date"
                                name="meet_date"
                                value="{{ data_get($order->meetup_location, 'date') }}"
                                class="form-control"
                                required
                              >
                            </div>
                            ...
                            <div class="form-group flex-fill">
                              ...
                              <input
                                id="buyer-time-{{ $order->id }}"
                                type="time"
                                name="meet_time"
                                value="{{ data_get($order->meetup_location, 'time') }}"
                                class="form-control"
                                required
                              >
                            </div>
                          ...
                          <input type="hidden" name="meet_lat" value="{{ data_get($order->meetup_location, 'lat') }}">
                          <input type="hidden" name="meet_lng" value="{{ data_get($order->meetup_location, 'lng') }}">
                  ...
                  @elseif($isCompleted)
                    ...
                    <div class="d-flex flex-column gap-2 align-items-end">
                      ...
                      @else
                        @php
                          $modalId = 'rating-modal-' . $order->id;
                        @endphp

                        <button
                          type="button"
                          class="btn btn-warning btn-sm text-end"
                          ...
                          <div class="modal__dialog" role="document">
                            ...
                            <div class="modal__body">
                              <form method="POST" action="{{ route('ratings.store', $seller) }}" class="stack gap-3">
                                ...
                                <div class="form-group">
                                  <label class="form-label" for="rating-comment-{{ $order->id }}">
                                    評價內容（選填）
                                  </label>
                                  <textarea
                                    id="rating-comment-{{ $order->id }}"
                                    name="comment"
                                    class="form-control"
                                    rows="3"
                                    maxlength="1000"
                                    placeholder="分享這次交易的感受吧！"
                                  ></textarea>
                                </div>
          ...
          <tbody>
            @foreach($sellerOrders as $order)
              @php
                  ...
                  $buyer = $order->user;
                  $buyerName = optional($buyer)->nickname
                      ?? optional($buyer)->account
                      ?? '使用者已刪除';
                  ...
                  @if($order->order_status === 'pending')
                    <div class="d-flex flex-column gap-2 align-items-end">
                      ...
                      <form
                        ...
                        onsubmit="return confirm('確認這筆訂單嗎？');"
                      >
                        @csrf
                        @method('PATCH')
